...
<div
    class="group relative flex aspect-2/3 overflow-hidden bg-zinc-100 dark:bg-white/10"
    @if (!$photo->portrait_thumbnail_url) wire:poll.visible.5s @endif
>
    <a
        id="{{ $htmlId }}"
        wire:navigate
    ...
    >
        @if ($photo->isImage())
            @if ($photo->portrait_thumbnail_url)
                <img
                    x-data="{ loaded: false, errored: false }"
                    x-init="if ($el.complete) loaded = true"
                    src="{{ $photo->portrait_thumbnail_url }}"
                    alt=""
                    @contextmenu.prevent
                    x-on:load="loaded = true"
                    x-on:error="errored = true"
                    class="h-full w-full bg-zinc-300 object-cover dark:bg-white/10 rounded"
